...
const Country = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.country?.countrylist?.data);
  const list_single = useSelector((state) => state?.country?.countrysingle?.data);
  ...
  const handleSubmit = async (values,actionType) => {
    if (actionType === "create") {
      ...
      await api.remove(
        dispatch,
        [countryStart, countrySuccess, countryFailure, "countrydelete"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [countryStart, countrySuccess, countryFailure, "countrydeletemany"],
        "mastersettings",
        { ids: values },
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [countryStart, countrySuccess, countryFailure, "countrysingle"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                {from_service.formview("Country Name", list_single?.name)}
