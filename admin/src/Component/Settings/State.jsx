...
const State = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.state?.statelist?.data);
  const clists = useSelector((state) => state?.country?.countrylist?.data);
  const list_single = useSelector(
    (state) => state?.state?.statesingle?.data
  );
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      await api.create(
        dispatch,
        [stateStart, stateSuccess, stateFailure, "stateadd"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [stateStart, stateSuccess, stateFailure, "statesingle"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
            ...
            setLoading((prev) => ({ ...prev, edit: false }));
            formEdit.setFieldsValue({...res?.data?.data,country:res?.data?.data.country?.name});
      ...
      await api.update(
        dispatch,
        [stateStart, stateSuccess, stateFailure, "stateedit"],
        "mastersettings",
        list_single?._id,
        values,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [stateStart, stateSuccess, stateFailure, "statedelete"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [stateStart, stateSuccess, stateFailure, "statedeletemany"],
        "mastersettings",
        { ids: values },
        (err, res) => {
          if (err) {
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                {from_service.formview("State Name", list_single?.name)}
