...
const Users = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.users?.userslist?.data);
  const list_single = useSelector((state) => state?.users?.userssingle?.data);
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      await api.create(
        dispatch,
        [usersStart, usersSuccess, usersFailure, "usersadd"],
        "buyer",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [usersStart, usersSuccess, usersFailure, "userssingle"],
        "buyer",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.update(
        dispatch,
        [usersStart, usersSuccess, usersFailure, "usersedit"],
        "buyer",
        list_single?._id,
        values,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [usersStart, usersSuccess, usersFailure, "usersdelete"],
        "buyer",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [usersStart, usersSuccess, usersFailure, "usersdeletemany"],
        "buyer",
        { ids: values },
        (err, res) => {
          if (err) {
    ...
    name:
      (
        <div className="table_img">
          <Image src={e.image?`http://localhost:9000/${e.image}` : dummy_profile} />
          <p>{e.name}</p>
        </div>
      ) ?? "",
    phone: e.phone ?? "",
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                ...
                {from_service.formview("Gender", list_single?.gender)}
