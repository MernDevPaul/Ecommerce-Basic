...
const Category = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.category?.categorylist?.data);
  const list_single = useSelector((state) => state?.category?.categorysingle?.data);
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      if (
        ...
        form_data.append("parent_id", values["parent_id"]);
      }
      ...
      await api.create(
        dispatch,
        [categoryStart, categorySuccess, categoryFailure, "categoryadd"],
        "mastersettings",
        form_data,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [categoryStart, categorySuccess, categoryFailure, "categorysingle"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
            ...
            setLoading((prev) => ({ ...prev, edit: false }));
            formEdit.setFieldsValue({ ...res?.data?.data, parent_id: res?.data?.data?.parent_id?._id });
            if (res?.data?.data?.image !== null && res?.data?.data?.image!=="") {
              setFileList([
                {
      ...
      form_data.append("name", values["name"]);
      form_data.append("status", Boolean(values["status"]));
      await api.update(
        dispatch,
        [categoryStart, categorySuccess, categoryFailure, "categoryedit"],
        "mastersettings",
        list_single?._id,
        form_data,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [categoryStart, categorySuccess, categoryFailure, "categorydelete"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [categoryStart, categorySuccess, categoryFailure, "categorydeletemany"],
        "mastersettings",
        { ids: values },
        (err, res) => {
          if (err) {
  ...
  const category_option = lists?.map((e) => ({
    label: e.name,
    value: e._id,
  }));
  return (
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                {from_service.formview("Category Name", list_single?.name)}
                ...
                {from_service.formview(
                  "Image",
                  <Image src={list_single?.image?`http://localhost:9000/${list_single?.image}`:default_img} className="t_image_35" />
                )}
                {from_service.formview(
                  "Status",
