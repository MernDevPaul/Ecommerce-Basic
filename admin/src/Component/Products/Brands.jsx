...
const Brands = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.brands?.brandslist?.data);
  const list_single = useSelector((state) => state?.brands?.brandssingle?.data);
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      form_data.append("type", "brands");
      await api.create(
        dispatch,
        [brandsStart, brandsSuccess, brandsFailure, "brandsadd"],
        "mastersettings",
        form_data,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [brandsStart, brandsSuccess, brandsFailure, "brandssingle"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.update(
        dispatch,
        [brandsStart, brandsSuccess, brandsFailure, "brandsedit"],
        "mastersettings",
        list_single?._id,
        form_data,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [brandsStart, brandsSuccess, brandsFailure, "brandsdelete"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [brandsStart, brandsSuccess, brandsFailure, "brandsdeletemany"],
        "mastersettings",
        { ids: values },
        (err, res) => {
          if (err) {
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                {from_service.formview("Brand Name", list_single?.name)}
