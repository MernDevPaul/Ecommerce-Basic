...
const Tax = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.tax?.taxlist?.data);
  const list_single = useSelector(
    (state) => state?.tax?.taxsingle?.data
  );
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      await api.create(
        dispatch,
        [taxStart, taxSuccess, taxFailure, "taxadd"],
        "tax",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [taxStart, taxSuccess, taxFailure, "taxsingle"],
        "tax",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.update(
        dispatch,
        [taxStart, taxSuccess, taxFailure, "taxedit"],
        "tax",
        list_single?._id,
        values,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [taxStart, taxSuccess, taxFailure, "taxdelete"],
        "tax",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [taxStart, taxSuccess, taxFailure, "taxdeletemany"],
        "tax",
        { ids: values },
        (err, res) => {
          if (err) {
  ...
  const columns = [
    ...
    },
    {
      title: "HSN Code",
      dataIndex: "hsn_code",
      ...from_service.tableService("hsn_code"),
    },
    {
      title: "Tax Persentage",
      dataIndex: "tax_persentage",
      ...from_service.tableService("tax_persentage"),
    },
    {
      title: "Status",
      dataIndex: "status",
          ...
          <>
            {loading.view ? (
              ...
              <div className="view_model_form">
                {from_service.formview("Tax Name", list_single?.name)}
                {from_service.formview("HSN Code", list_single?.hsn_code)}
                {from_service.formview("Tax Persentage", list_single?.tax_persentage)}
