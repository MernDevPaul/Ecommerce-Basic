...
const BlogCategory = ({ title }) => {
  ...
  const lists = useSelector((state) => state?.blogcategory?.blogcategorylist?.data);
  const list_single = useSelector((state) => state?.blogcategory?.blogcategorysingle?.data);
  ...
  const initFunction = async () => {
    ...
    await api.getAll(
      dispatch,
      [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategorylist"],
      "mastersettings",
      { type: "blogcategory" },
      (err, res) => {
  ...
  const handleSubmit = async (values, actionType) => {
    if (actionType === "create") {
      ...
      values["type"] = "blogcategory";
      await api.create(
        dispatch,
        [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategoryadd"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.getSingle(
        dispatch,
        [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategorysingle"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.update(
        dispatch,
        [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategoryedit"],
        "mastersettings",
        list_single?._id,
        values,
        (err, res) => {
      ...
      await api.remove(
        dispatch,
        [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategorydelete"],
        "mastersettings",
        values,
        (err, res) => {
          if (err) {
      ...
      await api.removeMany(
        dispatch,
        [blogcategoryStart, blogcategorySuccess, blogcategoryFailure, "blogcategorydeletemany"],
        "mastersettings",
        { ids: values },
        (err, res) => {
          if (err) {
