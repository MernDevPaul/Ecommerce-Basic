...
class SocialMediaController extends BaseController {
  ...
  async createHandler(req, res) {
    ...
    var _a;
    const check = await this.findOne(this.model, { name: req.body.name,company_code: req.body.company_code }, {}, {});
  ...
  async getSingle(req, res) {
    ...
    const { id } = req.params;
    validateId(id);
    const check = await this.findOne(this.model, { ...req.query,_id: id,company_code: company_code }, {}, {});
    if (!check) throw new Error("Not found");
  ...
  async getAll(req, res) {
    const { company_code } = req.admin;
    const check = await this.find(this.model, { ...req.query,company_code: company_code }, {}, {});
    success(res, 200, true, "Get Successfully", check);
  }
}

module.exports = new SocialMediaController().router;
